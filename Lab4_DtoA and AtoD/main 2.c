...
interrupt void ADCISR(void)
{
    ...
    adcU[cycles] = AdcaResultRegs.ADCRESULT0*Vref/4096;
    // 3. (Slow) Compute and store the next output voltage value.
    temp[0][0] = adcU[0];

    if (cycles+1 != 1000){
        temp[0][cycles+1] = temp[0][cycles] + A[0][0]*temp[0][cycles] +  A[0][1]*temp[1][cycles] + A[0][2]*temp[2][cycles] + B[0][0]*adcU[cycles];
    } else {
        temp[0][0] = temp[0][cycles] + A[0][0]*temp[0][cycles] +  A[0][1]*temp[1][cycles] + A[0][2]*temp[2][cycles] + B[0][0]*adcU[cycles];
    }
    dacY[cycles] = temp[0][cycles];
